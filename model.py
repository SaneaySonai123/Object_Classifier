...
class Model:
    ...
    def train_model(self, counters):
        ...
        for i in range(1, counters[2]):                            #change
            img = cv.imread(f'3/frame{i}.jpg')[:, :, 0]
            img = img.reshape(16950)
            img_list = np.append(img_list, [img])
            class_list = np.append(class_list, 3)

        img_list = img_list.reshape(counters[0] - 1 + counters[1] - 1 + counters[2] - 1, 16950)          #change
    ...
    def predict(self, frame):
        ...
        img = img.reshape(16950)
        prediction = self.model.predict([img])    #error
